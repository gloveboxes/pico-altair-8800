...
static void publish_virtual_input_data(uint16_t switches)
{
    ...
    uint8_to_binary((uint8_t)(switches >> 8), address_bus_high_byte, sizeof(address_bus_high_byte));
...
static void process_virtual_switches(const char* command)
{
    ...
    if (validate_input_data(command))
    {
        ...
        publish_virtual_input_data(bus_switches);
    }
}
...
{
    if (cpu_state_get_mode() == CPU_STOPPED || cmd_switches == STOP_CMD)
    {
        switch (cmd_switches)
        {
            case RUN_CMD:
